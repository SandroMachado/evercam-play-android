...
public class HikvisionSdk implements Callback
{
    ...
    private int loginDevice()
    {
        ...
        if(loginId < 0)
        {
            Log.e(TAG, "NET_DVR_Login is failed!Err:" + HCNetSDK.getInstance()
    ...
    private void play(NET_DVR_TIME beginTime)
    {
        ...
        else
        {
            ...
            Log.e(TAG, "NET_DVR_PlayBackByTime failed, error code: " + HCNetSDK.getInstance()
                    .NET_DVR_GetLastError());
        }
    }
    ...
    public void processRealData(int iPlayViewNo, int iDataType, byte[] pDataBuffer,
                                int iDataSize, int iStreamMode)
    {
        ...
        if(HCNetSDK.NET_DVR_SYSHEAD == iDataType)
        {
            ...
            if(iDataSize > 0)
            {
                ...
                if(!hikvisionPlayer.openStream(playPort, pDataBuffer, iDataSize,
                        2 * 1024 * 1024)) // open
                // stream
                {
