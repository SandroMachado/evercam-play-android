...
import io.evercam.androidapp.utils.Crypto;
...
public class DbAppUser extends DatabaseMaster
{
    ...
    {
        String createUserTableQuery = "CREATE TABLE " + TABLE_APP_USER + "(" + KEY_ID + " INTEGER" +
                " PRIMARY KEY autoincrement" + "," + KEY_EMAIL + " TEXT UNIQUE" + "," +
    ...
    public AppUser getAppUserByEmail(String email) throws NumberFormatException, Exception
    {
        ...
        Cursor cursor = db.query(TABLE_APP_USER, new String[]{KEY_ID, KEY_EMAIL, KEY_USERNAME,
                KEY_PASSWORD, KEY_API_KEY, KEY_API_ID, KEY_COUNTRY, KEY_IS_DEFAULT},
        ...
        if(cursor != null)
        {
            if(cursor.moveToFirst())
            {
                user = new AppUser(Integer.parseInt(cursor.getString(0)), cursor.getString(1),
                        cursor.getString(2), Crypto.decrypt(cursor.getString(3)),
    ...
    public AppUser getAppUserByID(int userId) throws NumberFormatException, Exception
    {
        ...
        Cursor cursor = db.query(TABLE_APP_USER, new String[]{KEY_ID, KEY_EMAIL, KEY_USERNAME,
                KEY_PASSWORD, KEY_API_KEY, KEY_API_ID, KEY_COUNTRY, KEY_IS_DEFAULT},
                KEY_ID + "=?", new String[]{userId + ""}, null, null, null, null);
        if(cursor != null)
        {
            if(cursor.moveToFirst())
            {
    ...
    public AppUser getAppUserByUsername(String username)
    {
        ...
        if(cursor != null)
        {
            if(cursor.moveToFirst())
            {
                try
                {
                    user = new AppUser(Integer.parseInt(cursor.getString(0)),
                            cursor.getString(1), cursor.getString(2),
                            Crypto.decrypt(cursor.getString(3)), cursor.getString(4),
        ...
        if(cursor.moveToFirst())
        {
            do
            {
                AppUser user = new AppUser(Integer.parseInt(cursor.getString(0)),
                        cursor.getString(1), cursor.getString(2), Crypto.decrypt(cursor.getString
                        (3)), cursor.getString(4), cursor.getString(5), cursor.getString(6),
                        Integer.parseInt(cursor.getString(7)));
