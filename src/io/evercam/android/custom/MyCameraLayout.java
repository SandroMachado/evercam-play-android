...
public class MyCameraLayout extends LinearLayout
{
	...
	public MyCameraLayout(Context context, EvercamCamera camera)
	{
		super(context);
...
//				{
//					AlertDialog.Builder builder = UIUtils.GetAlertDialogBuilderNoTitle(MyCameraLayout.this.context);
//					final View layout = ((CamerasActivity) MyCameraLayout.this.context).getLayoutInflater().inflate(
...
//						public void onClick(View v)
//						{
//							VideoActivity.startPlayingVIdeoForCamera(MyCameraLayout.this.context, evercamCamera.getCameraID());
//							dialog.cancel();
//						}
//					});
//
...
//						public void onClick(View v)
//						{
//							RVideoViewActivity.StartPlayingVIdeo(MyCameraLayout.this.context, evercamCamera, null);
//							dialog.cancel();
//						}
//					});
//
...
//							new Integer[] { evercamCamera.getCameraId() });
			...
			try
			{
				if (result != null && !end)
				{
					...
					if (d != null && d.getIntrinsicWidth() > 0 && d.getIntrinsicHeight() > 0)
					{
						...
						MyCameraLayout.this.evercamCamera.loadingStatus = ImageLoadingStatus.live_received;
					}
					else
					{
			...
			try
			{
				synchronized (this)
				{
					...
					if (liveImageTask.isTaskended
							...
							&& MyCameraLayout.this.evercamCamera.loadingStatus != ImageLoadingStatus.live_received) MyCameraLayout.this.evercamCamera.loadingStatus = ImageLoadingStatus.live_not_received;
	...
	private class DownloadLatestTask extends AsyncTask<Integer, Void, String>
	{
		...
		{ // DownloadImageLatestCamba
			try
			{
				if (result != null && !end)
				{
					...
					if (d != null && d.getIntrinsicWidth() > 0 && d.getIntrinsicHeight() > 0)
					{
						cameraRelativeLayout.setVisibility(View.VISIBLE);
						cameraRelativeLayout.setBackgroundDrawable(d);
						MyCameraLayout.this.evercamCamera.loadingStatus = ImageLoadingStatus.camba_image_received;
					}
					else
					{
						if (enableLogs) Log.e(TAG, "File Load Error" + "::"
			...
			try
			{
				if (MyCameraLayout.this.evercamCamera.loadingStatus != ImageLoadingStatus.camba_image_received) MyCameraLayout.this.evercamCamera.loadingStatus = ImageLoadingStatus.camba_not_received;
			}
			catch (Exception e)
			{
			}
