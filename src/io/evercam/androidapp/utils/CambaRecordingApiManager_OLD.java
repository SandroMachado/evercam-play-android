...
public class CambaRecordingApiManager_OLD
{
	...
	static String CambaRecordingApiBaseUrl = "https://cloud.camba.tv/WebService/MongoService.asmx";
	...
	public static boolean GetSnapshotsOptimized(ArrayList<ImageRecord> imageList, int cameraId,
			String timezone, String cameraGroup, String from, String to, String playFromTime,
			int iteration, int limit, String hourServerIp) throws Exception,
			IllegalArgumentException
	{
		...
		try
		{
			HttpClient client = new DefaultHttpClient();
			HttpPost post = new HttpPost(CambaRecordingApiBaseUrl + "/"
					+ CambaRecordingApi_GetSnapshotsOptimized);
			post.setHeader("Accept", "application/xml");
			post.setHeader("User-Agent", "Apache-HttpClient/4.1 (java 1.5)");
			...
			nvps.add(new BasicNameValuePair("from", from + ""));
			nvps.add(new BasicNameValuePair("to", to + ""));
			nvps.add(new BasicNameValuePair("playFromTime", playFromTime + ""));

			nvps.add(new BasicNameValuePair("iteration", iteration + ""));
			nvps.add(new BasicNameValuePair("limit", limit + ""));
			nvps.add(new BasicNameValuePair("hourServerIp", iteration + ""));
			...
			{ // count++;
				...
				if (eventType == XmlPullParser.START_TAG)
				{
					if (xpp.getName().toUpperCase(Locale.ENGLISH).equals("SI"))
					{
						obj = new ImageRecord();
					}
					if (eventType != XmlPullParser.TEXT)
