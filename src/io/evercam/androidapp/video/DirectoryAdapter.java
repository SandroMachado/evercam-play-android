...
public class DirectoryAdapter extends BaseAdapter
{
	...
	public void refresh()
	{
		...
		for (File f : files)
		{
			...
			if (f.getName().contains("."))
			{
				...
				if (i > 0)
				{
					if ((mAudio && Media.AUDIO_EXTENSIONS.contains(f.getName().substring(i)))
							|| (!mAudio && Media.VIDEO_EXTENSIONS
									.contains(f.getName().substring(i))))
					{
	...
	{
		return LibVLC.PathToURI(Environment.getExternalStorageDirectory().getAbsolutePath() + '/'
				+ mFiles.get(position));
	}
