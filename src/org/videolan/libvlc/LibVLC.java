...
public class LibVLC
{
	...
	static
	{
		try
		{
			if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.GINGERBREAD_MR1) System
					.loadLibrary("iomx-gingerbread");
			else if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.HONEYCOMB_MR2) System
					.loadLibrary("iomx-hc");
			else if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR1) System
					.loadLibrary("iomx-ics");
			else if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.JELLY_BEAN_MR2) System
					.loadLibrary("iomx-jbmr2");
			else if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT) System
					.loadLibrary("iomx-kk");
		}
		catch (Throwable t)
		{
			// No need to warn if it isn't found, when we intentionally don't
			// build these except for debug
			if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) Log.w(TAG,
					"Unable to load the iomx library: " + t);
		}
		try
		{
	 ...
	 * 
	 * @param mediaList
	 *            The media list object to follow
	 */
	...
	/**
	 * Gets the primary media list, or the "currently playing" list. Not to be
	 * confused with the media list pointer from above, which refers the the
	 * MediaList object that libVLC is currently following. This list is just
	 * one out of many lists that it can be pointed towards.
	 * 
	 * This list will be used for lists of songs that are not user-defined. For
	 * example: selecting a song from the Songs list, or from the list displayed
	...
	public void setHardwareAcceleration(int hardwareAcceleration)
	{
		...
		{
			// Automatic mode: activate MediaCodec opaque direct rendering for
			// 4.3 and above.
	...
	/**
	 * Pause the Java audio output This function is called by the native code
	 */
	 ...
	 * 
	 * @param mrl
	 *            MRL of the media to play.
	 */
	public void playMRL(String mrl)
	{
		// index=-1 will return options from libvlc instance without relying on
		// MediaList
	 ...
	 * 
	 * @return the index of the media was expanded, and -1 if no media was
	 *         expanded
	 */
