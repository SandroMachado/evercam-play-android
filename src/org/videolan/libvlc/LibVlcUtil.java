...
public class LibVlcUtil
{
	...
	public static boolean hasCompatibleCPU(Context context)
	{
		...
		String libBasePath;
		if ((applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) libBasePath = "/system";
		else libBasePath = applicationInfo.dataDir;
		...
		Log.i(TAG, "machine = "
				+ (elf.e_machine == EM_ARM ? "arm" : elf.e_machine == EM_386 ? "x86" : "mips"));
		...
		try
		{
			...
			while ((line = br.readLine()) != null)
			{
				...
				if (line.contains("GenuineIntel")) hasX86 = true;
				// "microsecond timers" is specific to MIPS.
				// see arch/mips/kernel/proc.c
				if (line.contains("microsecond timers")) hasMips = true;
				if (!hasNeon && (line.contains("neon") || line.contains("asimd"))) hasNeon = true;
				if (!hasFpu
						&& (line.contains("vfp") || (line.contains("Features") && line
								.contains("fp")))) hasFpu = true;
	...
	private static boolean readHeader(RandomAccessFile in, ElfData elf) throws IOException
	{
		...
		in.readFully(bytes);
		if (bytes[0] != 127 || bytes[1] != 'E' || bytes[2] != 'L' || bytes[3] != 'F'
				|| bytes[4] != 1)
				...
				while (buffer.position() < start_section + length)
				{
					...
					while (buffer.position() < start + size)
					{
						...
						else
						{
							...
							// uleb128 for other
							tag %= 128;
							if (tag == 4 || tag == 5 || tag == 32 || (tag > 32 && (tag & 1) != 0)) getString(buffer);
