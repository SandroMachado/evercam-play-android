...
public class MediaList
{
	...
	/*
	 * Since the libvlc_media_t is not created until the media plays, we have to
	 * cache them here.
	 */
	...
	private LibVLC mLibVLC; // Used to create new objects that require a libvlc
							// instance
	private EventHandler mEventHandler;

	public MediaList(LibVLC libVLC)
	{
		mEventHandler = new EventHandler(); // used in init() below to fire
											// events at the correct targets
	...
	public void add(Media media, boolean noVideo, boolean noHardwareAcceleration)
	{
		...
		signal_list_event(EventHandler.CustomMediaListItemAdded, mInternalList.size() - 1,
				media.getLocation());
	}
	...
	public void clear()
	{
		...
		{
			signal_list_event(EventHandler.CustomMediaListItemDeleted, i,
					mInternalList.get(i).m.getLocation());
		}
		mInternalList.clear();
	}
	...
	/**
	 * This function checks the currently playing media for subitems at the
	 * given position, and if any exist, it will expand them at the same
	 * position and replace the current media.
	 * 
	 * @param position
	...
	{
		if (!(isValid(startPosition) && endPosition >= 0 && endPosition <= mInternalList.size())) throw new IndexOutOfBoundsException(
	...
	public String[] getMediaOptions(int position)
	{
		...
		if (isValid(position))
		{
			if (!noHardwareAcceleration) noHardwareAcceleration = mInternalList.get(position).noHardwareAcceleration;
		...
		if (!noHardwareAcceleration)
		{
			 ...
			 * enough to make the decoder output a frame, the playback timing
			 * gets started too soon, and every decoded frame appears to be too
			 * late. On Nexus One, the decoder latency seems to be 25 input
			 * packets for 320x170 H.264, a few packets less on higher
			 * resolutions. On Nexus S, the decoder latency seems to be about 7
			 * packets.
			 */
			...
			options.add(":rtsp-tcp"); // 1
			options.add(":no-ffmpeg-hw");

			// options.add(":rtsp-mcast");
			// options.add(":sharpen-sigma=2.0");
			// options.add(":blur-factor=127");
			options.add(":ipv4-timeout=2500"); // 1
			// options.add("--no-video-title-show");
			options.add(":no-video-title-show");
			// options.add(":sout-all");
			options.add(":sout-keep"); // 1

			// https://forum.videolan.org/viewtopic.php?f=4&t=87805 sajjad
		}
